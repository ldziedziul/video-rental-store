...
class DefaultRentalServiceTest extends Specification {
    ...
    def "should rent films"() {
        ...
        def tested = new DefaultRentalService(Mock(CustomerService), rentalRepository, rentalCreator, Mock(RentedFilmRepository), Mock(SurchargeCalculator), Mock(ApplicationEventPublisher))
    ...
    def "should fail when customer not found"() {
        ...
        def tested = new DefaultRentalService(customerService, Mock(RentalRepository), Mock(RentalEntityCreator), Mock(RentedFilmRepository), Mock(SurchargeCalculator), Mock(ApplicationEventPublisher))
        when:
    ...
    def "should return films and calculate surcharge"() {
        ...
        def tested = new DefaultRentalService(Mock(CustomerService), Mock(RentalRepository), Mock(RentalEntityCreator), rentedFilmRepository, surchargeCalculator, Mock(ApplicationEventPublisher))
        when:
    ...
    def "should fail when trying to return not rented film"() {
        ...
        def tested = new DefaultRentalService(Mock(CustomerService), Mock(RentalRepository), Mock(RentalEntityCreator), rentedFilmRepository, Mock(SurchargeCalculator), Mock(ApplicationEventPublisher))
        when:
